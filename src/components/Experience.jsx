...
export default function Experience() {
  return (
    <section className="mx-auto w-[min(1120px,calc(100%-10vw))] py-[72px]" id="experience">
      <Reveal>
        <p className="mb-[8px] text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-[var(--accent)]">03 — Career</p>
      ...
      <div className="mt-[28px] grid gap-4">
        {experience.map((job, index) => (
          <Reveal key={job.company} delay={index * 90} direction={index % 2 === 0 ? 'right' : 'left'}>
      ...
      <div className="mt-[56px]">
        <Reveal direction="left">
          <div className="mb-[16px]">
            <p className="mb-[8px] text-[0.78rem] font-semibold uppercase tracking-[0.16em] text-[var(--accent)]">04 — Learning</p>
            <h3 className="text-[clamp(1.8rem,3vw,2.6rem)] font-extrabold leading-[1.15] tracking-[-0.03em] text-[var(--text)]">Education</h3>
          </div>
        </Reveal>

        <div className="grid gap-4">
          {education.map((item, index) => (
            <Reveal key={item.title} delay={index * 90} direction={index % 2 === 0 ? 'right' : 'down'}>
              <article className="grid gap-4 md:grid-cols-[18px_1fr]">
                <div className="mt-[28px] h-[14px] w-[14px] rounded-full bg-[var(--accent)] shadow-[0_0_0_6px_var(--glow)]" />
                <div className="rounded-[22px] border border-[var(--line)] bg-[var(--surface)] p-6 shadow-[var(--card-shadow)] transition-transform duration-300 hover:-translate-y-1.5 hover:border-[color:color-mix(in_srgb,var(--accent)_45%,var(--line))]">
                  <div className="flex flex-wrap justify-between gap-3">
                    <div>
                      <h3 className="mb-[4px] text-[1.2rem] font-bold text-[var(--text)]">{item.title}</h3>
                      <p className="text-[var(--muted)]">{item.school}</p>
                    </div>
                    <p className="text-[var(--muted)]">{item.period}</p>
                  </div>
                  {item.meta ? <p className="mt-[12px] inline-flex rounded-full border border-[var(--line)] bg-[var(--bg-soft)] px-[10px] py-[6px] text-[0.8rem] font-medium text-[var(--text)]">{item.meta}</p> : null}
                </div>
              </article>
            </Reveal>
          ))}
